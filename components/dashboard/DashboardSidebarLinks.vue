        ...
        <component :is="link.children?.length ? DisclosureButton : 'div'" as="template">
          ...
          >
            <ULink
              v-slot="{ isActive }"
              v-bind="
                link.children?.length && link.collapsible !== false ? { disabled: link.disabled } : getULinkProps(link)
              "
              :class="[ui.base, isDragging && 'pointer-events-none']"
              :active-class="ui.active"
              :inactive-class="
                !link.to && link.collapsible === false && level === 0 && link.children?.length ? ui.static : ui.inactive
              "
              draggable="false"
              @click="link.click"
            >
                ...
                <UIcon
                  ...
                  :class="
                    twMerge(
                      twJoin(
                        ui.icon.base,
                        isActive ? ui.icon.active
                        : !link.to && link.collapsible === false && level === 0 && link.children?.length ? ui.static
                        : ui.icon.inactive
                      ),
                      link.iconClass
                    )
